...
def get_updated_version():
    ...
    return '-'.join(base_version_components + [get_updated_revision()])


class AptPackage(threading.Thread):
    """
    A class to obtain the updated SSH ServerVersion of the Ubuntu openssh package.

    Create one instance only, as it starts a thread to periodically update.
    """
    ...
    def refresh_version(self):
        self.version = get_updated_version()
